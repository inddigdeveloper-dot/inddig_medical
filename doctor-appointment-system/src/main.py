...
def register_doctor(doctor: DoctorCreate, session: Session = Depends(get_db)):
    ...
    booking_link = f"http://localhost:5173/book/{new_doctor.username}"
...
async def delete_appointment(appointment_id: int, current_doctor: db.Doctor = Depends(auth.get_current_doctor), session: Session = Depends(get_db)):
    apt = session.query(db.Appointment).filter(db.Appointment.id == appointment_id).first()
    if not apt:
        raise HTTPException(status_code=404, detail="Appointment not found.")
