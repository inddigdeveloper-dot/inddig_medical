...
async def google_callback(state: str, code: str, session: Session = Depends(get_db)):
    ...
    if not refresh_token:
        return{"message": "Already connected to google. Please close this window."}
    ...
    session.commit()
    return{"message": " google calendar4 vconnected successfully! You can close this window."}
...
def add_to_calendar(name, email, doctor_email, booking_date, slot_time, custom_message, user_timezone="UTC"):
    service = get_service_calendar(doctor_refresh_token)
